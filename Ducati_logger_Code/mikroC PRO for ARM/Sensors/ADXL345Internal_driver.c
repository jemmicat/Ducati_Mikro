...
static void ADXL345_Write(unsigned short address, unsigned short data1) {
  ...
  data_[0] = address;
  data_[1] = data1;
  I2C2_Write(_ACCEL_ADDRESS, data_, 2, END_MODE_STOP);
}
...
void ADXL345_Read(int *data_X, int *data_Y, int *data_Z){
  data_[0] = _DATAX0;
  I2C2_Start();              // issue I2C start signal
  I2C2_Write(_ACCEL_ADDRESS, data_, 1, END_MODE_RESTART);
  I2C2_Read(_ACCEL_ADDRESS, data_, 6, END_MODE_STOP);
...
static unsigned short ADXL345_Read_Register(unsigned short address) {
  ...
  data_[0] = address;
  I2C2_Write(_ACCEL_ADDRESS, data_, 1, END_MODE_RESTART);
  I2C2_Read(_ACCEL_ADDRESS, data_, 1, END_MODE_STOP);
  return data_[0];
}
